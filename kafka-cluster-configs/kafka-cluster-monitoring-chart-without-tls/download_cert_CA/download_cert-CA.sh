...
kubectl get secret kafka-cluster-my-cluster-dev-cluster-ca-cert -n kafka -o jsonpath='{.data.ca\.crt}' | base64 --decode > ca.crt
kubectl get secret kafka-cluster-my-cluster-dev-cluster-ca-cert -n kafka -o jsonpath='{.data.ca\.password}' | base64 --decode > ca.password

kubectl get secret kafka-cluster-my-cluster-dev-cluster-ca-cert -n kafka -o jsonpath='{.data.ca\.p12}' | base64 --decode > ca.p12
...
kubectl get secret my-user -n kafka -o jsonpath='{.data.user\.p12}' | base64 --decode > user.p12
kubectl get secret my-user -n kafka -o jsonpath='{.data.user\.password}' | base64 --decode > user.password
